...
#define MINPASSWORD 1 // Colocado aqui para uma alteração mais fácil (melhor usar 0 ou 1, o tamanho entre o maximo e o minimo nunca muda)
...
int main() {
    ...
    do {
        ...
        if (sscanf(buffer, "%d", &op) != 1) { // Lê o que você digitou, guarda em um erray com o "fgets" e depois é analisado para ver se está de acordo com o tipo de formato
...
void printCor(HANDLE hConsole, const char *texto, int cor, int corF, ...) {
    ...
    // Esse negoço aqui em baixo eu n consegui fazer sozinho
    va_list args;
    va_start(args, corF);
    vprintf(texto, args);
    va_end(args);
